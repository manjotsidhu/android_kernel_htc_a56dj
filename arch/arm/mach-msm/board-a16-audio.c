...
#if 0 //8928 a16dwg has single speaker
...
//htc audio ++
static void htc_spk_booster_ctl(int enable)
{
	int i, value = (enable)?1:0;
...
static int msm8226_spk_booster_event(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *k, int event)
{
	pr_debug("%s()\n", __func__);
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		htc_spk_booster_ctl(1);
		/* for HW spec design, 5V booster need 1ms to raise output
		   voltage to 5V before the current on load circuit over the
		   protection threadhood.
		*/
		mdelay(2);
	} else {
		pr_info("%s: SND_SOC_DAPM_EVENT_OFF \n", __func__);

		htc_spk_booster_ctl(0);
	}
	mutex_unlock(&htc_amp_mutex);
...
//htc audio ++
	SND_SOC_DAPM_SPK("SPK BOOSTER", msm8226_spk_booster_event),
//htc audio --
};

//htc audio ++
static const struct snd_soc_dapm_route htc_spk_booster_switch_control[] = {
	{"SPK BOOSTER", NULL, "SPK_OUT"},
};
//htc audio --
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0])
		...
		htc_amp_mask &= ~HTC_SPK_AMP;
//htc audio ++
	msm8226_ext_spk_pamp |= LO_1_SPK_AMP;
//htc audio --
	htc_amp_control(htc_amp_mask,msm8226_ext_spk_pamp);
...
//htc audio ++
	snd_soc_dapm_add_routes(dapm, htc_spk_booster_switch_control,
		ARRAY_SIZE(htc_spk_booster_switch_control));
