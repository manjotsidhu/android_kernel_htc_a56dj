...
#define PANEL_ID_A16_TRULY_HX8394D 3
#define PANEL_ID_A16_SUCCESS_HX8394D 4
#define PANEL_ID_A16_TRULY_HX8394D_A31B 5
...
int htc_a16_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct dsi_power_data *pwrdata = NULL;
	u8 avdd_level = 0x00;
	...
	if (enable) {
		...
		if ((pdata->panel_info.panel_id == PANEL_ID_A16_TRULY_HX8394F) ||
			...
			usleep_range(180000,180500);
		} else if (pdata->panel_info.panel_id == PANEL_ID_A16_TRULY_HX8394D_A31B) {
			usleep_range(1000,1500);
			gpio_set_value((ctrl_pdata->rst_gpio), 1);
...
static int htc_a16_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		if ((pdata->panel_info.panel_id == PANEL_ID_A16_TRULY_HX8394F) ||
				...
				(pdata->panel_info.panel_id == PANEL_ID_A16_TRULY_HX8394D) ||
				(pdata->panel_info.panel_id == PANEL_ID_A16_SUCCESS_HX8394D) ||
				(pdata->panel_info.panel_id == PANEL_ID_A16_TRULY_HX8394D_A31B)) {
