...
#include <linux/CwMcuSensor_iio.h>
...
#ifdef CONFIG_AK8789_HALLSENSOR_R2
			   ...
			   size_t count, u8 reg_addr, u8 len)
{
	...
	D(
	  "%s: count = %zu, strlen(buf) = %zu, PAGE_SIZE = %lu,"
	  " reg_addr = 0x%x\n",
...
static void iio_trigger_work(struct irq_work *work)
{
	...
	if (atomic_read(&mcu_data->enter_suspend)) {
		W("%s: skip calling iio_trigger_poll, enter_suspend=%d\n", __func__, atomic_read(&mcu_data->enter_suspend));
		return;
	}
...
static void cwmcu_one_shot(struct work_struct *work)
{
	...
	I(
	  "w_activated_i2c is %d,w_re_init is %d, w_kick_start_mcu is %d, "
	  "w_mcu_state_change is %d, w_flush_fifo is %d, w_clear_fifo is %d , "
#ifdef CONFIG_AK8789_HALLSENSOR_R2
	  "w_hall_inform_mcu is %d, "
#endif
	  "w_batch_read is %d\n",
	  mcu_data->w_activated_i2c,mcu_data->w_re_init,
	  mcu_data->w_kick_start_mcu,mcu_data->w_mcu_state_change,
	  mcu_data->w_flush_fifo,mcu_data->w_clear_fifo,
#ifdef CONFIG_AK8789_HALLSENSOR_R2
	  mcu_data->w_hall_inform_mcu,
#endif
	  mcu_data->w_batch_read);
