...
static int himax_loadSensorConfig(struct i2c_client *client, struct himax_i2c_platform_data *pdata)
{
	...
	if (config_selected) {
		if (private_ts->chosen_config_process == LOAD_SHORT_CFG) {
			...
			himax_config_reg_write(client, 0x00, config_selected->c40, CUSTOMER_INFO_LENGTH, normalRetry);
...
int himax_ts_register_interrupt(struct i2c_client *client)
{
	...
	if (!ts->use_irq) {
		...
		I ("%s: ts->use_irq is set, do not use polling mode.\n", __func__);
	}
	return ret;
}
#endif
...
static int himax852xes_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct himax_i2c_platform_data *pdata;
	uint8_t buf[1] = {0};
#ifndef CONFIG_HTC_HELPER_FUNCTIONS
	char* mode[] = {
		"MFG_MODE_NORMAL",
		"MFG_MODE_FACTORY2",
		"MFG_MODE_RECOVERY",
				...
				unsigned long event, void *data)
{
	...
	I(" %s\n", __func__);
	if (evdata && evdata->data && event == FB_EVENT_PREBLANK && ts &&
	    ts->client) {
		blank = evdata->data;
		switch (*blank) {
