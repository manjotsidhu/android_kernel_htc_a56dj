...
int himax_touch_fw_update(struct firmware *vfw)
{
	...
	//Check fw header
	scnprintf(version, sizeof(version), "%02X", ts->vendor_fw_ver_H);
	I("[FW] version = %s\n", version);
...
static void himax_set_cover_work_func(struct work_struct *dummy)
{
	struct himax_ts_data *ts = private_ts;
	himax_set_cover_func(ts->cover_enable);
	I("[HL] cover_enable = %d.\n", ts->cover_enable);
}
static DECLARE_DELAYED_WORK(himax_set_cover_work, himax_set_cover_work_func);
			  ...
			  struct himax_i2c_platform_data *pdata)
{
	...
	I(" DT:gpio_irq=%d, gpio_rst=%d, gpio_3v3_en=%d, gpio_switch=%d\n", pdata->gpio_irq, pdata->gpio_reset, pdata->gpio_3v3_en, pdata->gpio_switch);
...
#ifdef CONFIG_SYNC_TOUCH_STATUS
	//Switch interrupt from edge trigger to level trigger before switch to sensor hub
...
#if defined(CONFIG_AK8789_HALLSENSOR) || defined(CONFIG_AK8789_HALLSENSOR_R2)
	if(ts->cover_support)
		schedule_delayed_work(&himax_set_cover_work, msecs_to_jiffies(50));
#endif
#endif
	himax_int_enable(ts->client->irq, 1);
