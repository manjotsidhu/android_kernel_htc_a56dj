...
#define ov13850_A16_SENSOR_NAME "ov13850_A16"
DEFINE_MSM_MUTEX(ov13850_A16_mut);

static struct msm_sensor_ctrl_t ov13850_A16_s_ctrl;

static struct msm_sensor_power_setting ov13850_A16_power_setting[] = {
	...
	#ifdef VCM_PWD_NC
		/* GPIO VCM Power Down no connected, skip */
	#else
	{
...
static struct v4l2_subdev_info ov13850_A16_subdev_info[] = {
	{
...
static const struct i2c_device_id ov13850_A16_i2c_id[] = {
	{ov13850_A16_SENSOR_NAME, (kernel_ulong_t)&ov13850_A16_s_ctrl},
	{ }
};

static int32_t msm_ov13850_A16_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	return msm_sensor_i2c_probe(client, id, &ov13850_A16_s_ctrl);
}

static struct i2c_driver ov13850_A16_i2c_driver = {
	.id_table = ov13850_A16_i2c_id,
	.probe  = msm_ov13850_A16_i2c_probe,
	.driver = {
		.name = ov13850_A16_SENSOR_NAME,
	},
};

static struct msm_camera_i2c_client ov13850_A16_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_WORD_ADDR,
};

static const struct of_device_id ov13850_A16_dt_match[] = {
	{.compatible = "htc,ov13850_A16", .data = &ov13850_A16_s_ctrl},
	{}
};

MODULE_DEVICE_TABLE(of, ov13850_A16_dt_match);

static struct platform_driver ov13850_A16_platform_driver = {
	.driver = {
		.name = "htc,ov13850_A16",
		.owner = THIS_MODULE,
		.of_match_table = ov13850_A16_dt_match,
	},
};

static const char *ov13850_A16Vendor = "OmniVision";
static const char *ov13850_A16NAME = "ov13850_A16";
static const char *ov13850_A16Size = "13.0M";
		...
		struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf, "%s %s %s\n", ov13850_A16Vendor, ov13850_A16NAME, ov13850_A16Size);
	ret = strlen(buf) + 1;
...
static struct kobject *android_ov13850_A16;

static int ov13850_A16_sysfs_init(void)
{
	int ret ;
	pr_info("ov13850_A16:kobject creat and add\n");
	android_ov13850_A16 = kobject_create_and_add("android_camera", NULL);
	if (android_ov13850_A16 == NULL) {
		pr_info("ov13850_A16_sysfs_init: subsystem_register " \
		"failed\n");
		ret = -ENOMEM;
		return ret ;
	}
	pr_info("ov13850_A16:sysfs_create_file\n");
	ret = sysfs_create_file(android_ov13850_A16, &dev_attr_sensor.attr);
	if (ret) {
		pr_info("ov13850_A16_sysfs_init: sysfs_create_file " \
		"failed\n");
		kobject_del(android_ov13850_A16);
	}

	return 0 ;
}

static int32_t ov13850_A16_platform_probe(struct platform_device *pdev)
{
	int32_t rc = 0;
	const struct of_device_id *match;
	match = of_match_device(ov13850_A16_dt_match, &pdev->dev);
...
static int __init ov13850_A16_init_module(void)
{
	int32_t rc = 0;
	pr_info("%s:%d\n", __func__, __LINE__);
	rc = platform_driver_probe(&ov13850_A16_platform_driver,
		ov13850_A16_platform_probe);
	if (!rc) {
		ov13850_A16_sysfs_init();
		return rc;
	}
	pr_err("%s:%d rc %d\n", __func__, __LINE__, rc);
	return i2c_add_driver(&ov13850_A16_i2c_driver);
}

static void __exit ov13850_A16_exit_module(void)
{
	pr_info("%s:%d\n", __func__, __LINE__);
	if (ov13850_A16_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&ov13850_A16_s_ctrl);
		platform_driver_unregister(&ov13850_A16_platform_driver);
	} else
		i2c_del_driver(&ov13850_A16_i2c_driver);
	return;
}

/*HTC_START*/
static int ov13850_A16_read_fuseid(struct sensorb_cfg_data *cdata,
	struct msm_sensor_ctrl_t *s_ctrl)
{
	#define ov13850_A16_LITEON_OTP_SIZE 0xF
 
	const short addr[3][ov13850_A16_LITEON_OTP_SIZE] = {
	...
	static uint8_t otp[ov13850_A16_LITEON_OTP_SIZE];
...
//HTC_END
		...
		// start from layer 2
		for (j=2; j>=0; j--) {
			for (i=0; i<ov13850_A16_LITEON_OTP_SIZE; i++) {
		...
		ov13850_A16_s_ctrl.driver_ic = otp[3];
...
//HTC_END
		...
		pr_info("ov13850_A16: fuse->fuse_id : 0x%x 0x%x 0x%x 0x%x\n",
...
//HTC_START , move read OTP to sensor probe
int32_t ov13850_A16_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	int32_t rc1 = 0;
	static int first = 0;
	...
	if(rc == 0) {
		if(first == 0) {
			...
			rc1 = ov13850_A16_read_fuseid(NULL, s_ctrl);
		}
	}
	first = 1;
	return rc;
...
static struct msm_sensor_fn_t ov13850_A16_sensor_func_tbl = {
	.sensor_config = msm_sensor_config,
	.sensor_power_up = msm_sensor_power_up,
	.sensor_power_down = msm_sensor_power_down,
    //HTC_START , move read OTP to sensor probe
	.sensor_match_id = ov13850_A16_sensor_match_id,
	//HTC_END
	.sensor_i2c_read_fuseid = ov13850_A16_read_fuseid,
};
/*HTC_END*/

static struct msm_sensor_ctrl_t ov13850_A16_s_ctrl = {
	.sensor_i2c_client = &ov13850_A16_sensor_i2c_client,
	.power_setting_array.power_setting = ov13850_A16_power_setting,
	.power_setting_array.size = ARRAY_SIZE(ov13850_A16_power_setting),
	.msm_sensor_mutex = &ov13850_A16_mut,
	.sensor_v4l2_subdev_info = ov13850_A16_subdev_info,
	.sensor_v4l2_subdev_info_size = ARRAY_SIZE(ov13850_A16_subdev_info),
	.func_tbl = &ov13850_A16_sensor_func_tbl,
};

module_init(ov13850_A16_init_module);
module_exit(ov13850_A16_exit_module);
MODULE_DESCRIPTION("ov13850_A16");
