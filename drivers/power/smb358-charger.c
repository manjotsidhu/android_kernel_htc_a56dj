...
#define OTG_CURRENT_LIMIT_MASK		SMB358_MASK(3, 2)
#define OTG_CURRENT_LIMIT_500		0x4
...
#define SMB358_CHG_I_HIGHVOL_MIN_MA	1300
...
static int smb358_float_voltage_set(struct smb358_charger *chip, int vfloat_mv)
{
	...
	if (rc)
		dev_err(chip->dev,
			"Couldn't read VFLOAT_REG rc=%d, reg=%2x\n",
							rc, VFLOAT_REG);
	else {
		reg &= VFLOAT_MASK;

		if (temp == reg) {
			pr_debug("skip the vfloat setting due to the new vfloat=%dmV"
				" is the same with the register value\n", vfloat_mv);
			return 0;
		}
	}
#endif
...
static int smb358_otg_ocp_threshold_set(struct smb358_charger *chip)
{

	int rc = 0;
	

	rc = smb358_masked_write(chip, OTG_TLIM_THERM_CTRL_REG,
				OTG_CURRENT_LIMIT_MASK, OTG_CURRENT_LIMIT_500);
	if (rc) {
		dev_err(chip->dev, "Couldn't set OTG_TLIM_THERM_CTRL_REG rc=%d\n",
				rc);
		return rc;
	}
...
static int smb358_regulator_init(struct smb358_charger *chip)
{
	...
	if (init_data->constraints.name) {
		...
		chip->otg_vreg.rdev = regulator_register(
			&chip->otg_vreg.rdesc, chip->dev,
			init_data, chip, chip->dev->of_node);
				...
				struct smb358_charger *chip)
{
	...
	unsigned int now_chg_limit_current = 0;
	...
	if (chg_limit_current[0] != 0 || chg_limit_current[1] != 0){
		if(chg_limit_current[0] != 0)
			now_chg_limit_current = chg_limit_current[0];
		else
			now_chg_limit_current = chg_limit_current[1];

		current_max = min(current_max, now_chg_limit_current);
	}
...
if(!flag_keep_charge_on && !flag_disable_temp_protection){
	...
	if (flag_keep_charge_on
			...
			|| (!chip->batt_cool && !chip->batt_warm && !chip->batt_hot)) {
		int vbat_mv = 0;
		...
		if (chip->vfloat_mv == 4420 && vbat_mv >= (4200 + 100))
			chip->vfloat_mv = 4400;
		else if (chip->vfloat_mv == 4400 && vbat_mv < 4200)
			chip->vfloat_mv = 4420;
...
#ifdef CONFIG_HTC_BATT_8960
int smb358_set_chg_iusbmax(int val)
{
	
	return -ENXIO;
}

int smb358_set_chg_vin_min(int val)
{
	
	return -ENXIO;
}
int smb358_get_chg_usb_iusbmax(void)
{
	
	return -ENXIO;
}
int smb358_get_chg_vinmin(void)
{
	
	return -ENXIO;
}
int smb358_get_input_voltage_regulation(void)
{
...
int smb358_get_batt_voltage(int *result)
{
	if (!the_chip) {
		pr_warn("called before init\n");
		return -EINVAL;
	...
	*result = smb358_get_prop_battery_voltage_now(the_chip)/1000;
...
void smb358_set_charger_configuration(void)
{
	...
	smb358_otg_ocp_threshold_set(chip);
...
int smb358_batt_warm_and_cool_recheck(void)
{
	bool bat_warm = 0, bat_cool = 0, bat_cold = 0, bat_hot = 0, adc_notify_fail = false;
	int temp = 0;
	struct smb358_charger *chip = the_chip;
	...
	if(temp >= chip->hot_bat_decidegc && !chip->batt_hot) {
		
		pr_info("[Go to become Hot]batt_temp:%d, bat_is_cool:%d, bat_is_warm:%d, bat_is_hot:%d, bat_is_cold:%d\n",
		temp, chip->batt_cool, chip->batt_warm, chip->batt_hot, chip->batt_cold);
		bat_warm = false;
		bat_cool = false;
		bat_hot = true;
		bat_cold = false;
	...
	} else if(temp >= (chip->warm_bat_decidegc - HYSTERESIS_DECIDEGC) && temp < (chip->hot_bat_decidegc - HYSTERESIS_DECIDEGC)
						&& chip->batt_hot){
		
		pr_info("[Hot turn to Warm]batt_temp:%d, bat_is_cool:%d, bat_is_warm:%d, bat_is_hot:%d, bat_is_cold:%d\n",
                temp, chip->batt_cool, chip->batt_warm, chip->batt_hot, chip->batt_cold);
                bat_warm = true;
                bat_cool = false;
                bat_hot = false;
	...
	} else if (temp >= chip->warm_bat_decidegc && temp < chip->hot_bat_decidegc && !chip->batt_warm && !chip->batt_hot) {
                                   
                pr_info("[Normal/Cool/Cold to Warm]batt_temp:%d, bat_is_cool:%d, bat_is_warm:%d, bat_is_hot:%d, bat_is_cold:%d\n",
                temp, chip->batt_cool, chip->batt_warm, chip->batt_hot, chip->batt_cold);
                bat_warm = true;
                bat_cool = false;
                bat_hot = false;
	...
	} else if(temp <= chip->cold_bat_decidegc && !chip->batt_cold){
		
		pr_info("[Go to become Cold]batt_temp:%d, bat_is_cool:%d, bat_is_warm:%d, bat_is_hot:%d, bat_is_cold:%d\n",
		temp, chip->batt_cool, chip->batt_warm, chip->batt_hot, chip->batt_cold);
		bat_warm = false;
		bat_cool = false;
		bat_hot = false;
        ...
        } else if(temp > (chip->cold_bat_decidegc + HYSTERESIS_DECIDEGC) && temp <= (chip->cool_bat_decidegc + HYSTERESIS_DECIDEGC)  							&& chip->batt_cold){
                
                pr_info("[Cold turn to Cool]batt_temp:%d, bat_is_cool:%d, bat_is_warm:%d, bat_is_hot:%d, bat_is_cold:%d\n",
                temp, chip->batt_cool, chip->batt_warm, chip->batt_hot, chip->batt_cold);
                bat_warm = false;
                bat_cool = true;
                bat_hot = false;
	...
	} else if(temp <= chip->cool_bat_decidegc && temp > chip->cold_bat_decidegc && !chip->batt_cool && !chip->batt_cold) {
                           
                pr_info("[Hot/Warm/Normal to Cool]batt_temp:%d, bat_is_cool:%d, bat_is_warm:%d, bat_is_hot:%d, bat_is_cold:%d\n",
                temp, chip->batt_cool, chip->batt_warm, chip->batt_hot, chip->batt_cold);
                bat_warm = false;
                bat_cool = true;
                bat_hot = false;
	...
	} else if(temp < (chip->warm_bat_decidegc - HYSTERESIS_DECIDEGC) && (chip->batt_warm || chip->batt_hot)) {
		
		pr_info("[Warm/Hot to Normal]batt_temp:%d, bat_is_cool:%d, bat_is_warm:%d, bat_is_hot:%d, bat_is_cold:%d\n",
		temp, chip->batt_cool, chip->batt_warm, chip->batt_hot, chip->batt_cold);
		bat_warm = false;
		bat_cool = false;
		bat_hot = false;
	...
	} else if (temp > (chip->cool_bat_decidegc + HYSTERESIS_DECIDEGC) && (chip->batt_cool || chip->batt_cold)) {
		
		pr_info("[Cool/Cold to Normal]batt_temp:%d, bat_is_cool:%d, bat_is_warm:%d, bat_is_hot:%d, bat_is_cold:%d\n",
		temp, chip->batt_cool, chip->batt_warm, chip->batt_hot, chip->batt_cold);
		bat_warm = false;
		bat_cool = false;
		bat_hot = false;
	...
	if (adc_notify_fail) {
		...
		if (bat_hot ^ chip->batt_hot || bat_cold ^ chip->batt_cold) {
			...
			if ((bat_hot || bat_cold || chip->battery_missing) && !flag_keep_charge_on && !flag_disable_temp_protection)
#ifndef CONFIG_HTC_BATT_8960
				smb358_charging_disable(chip, THERMAL, 1);
#else
				smb358_chg_disable_charger(chip, 1,
		...
		pr_info("batt_temp:%d, bat_is_cool:%d, bat_is_warm:%d, bat_is_hot:%d, bat_is_cold:%d\n",
		temp, chip->batt_cool, chip->batt_warm, chip->batt_hot, chip->batt_cold);
	} 
...
int smb358_is_batt_temp_fault_disable_chg(int *result)
{
	...
	smb358_batt_warm_and_cool_recheck();
	is_warm = chip->batt_warm;
	is_cool = chip->batt_cool;
	...
	pr_info("is_cold=%d, is_hot=%d, is_warm=%d, is_vbatt_over_vddmax=%d, warm_bat_mv:%d\n",
...
static int smb358_load_battery_data(struct smb358_charger *chip)
{
	...
	if ((batt_data->fastchg_current_max_ma >= 0) && (batt_data->fastchg_current_max_ma != -1)) {
		chip->fastchg_current_max_ma = batt_data->fastchg_current_max_ma;
		if((chip->fastchg_current_max_ma > ATS_IBAT_LIMIT) && (flag_ats_limit_chg)) {
			chip->fastchg_current_max_ma = ATS_IBAT_LIMIT;
		}
	}
	if ((batt_data->warm_bat_ma >= 0) && (batt_data->warm_bat_ma != -1))
		chip->warm_bat_ma = batt_data->warm_bat_ma;
	if ((batt_data->cool_bat_ma >= 0) && (batt_data->cool_bat_ma != -1))
...
int smb358_set_limit_charge_on_high_vol_enable(bool enable)
{
	pr_info("Enter high vol limit ibat function\n");
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	if(enable)
		chg_limit_current[1] = SMB358_CHG_I_HIGHVOL_MIN_MA;
	else
		chg_limit_current[1] = 0;
...
int smb358_limit_charge_enable(bool enable)
{
	...
	if (enable)
		chg_limit_current[0] = SMB358_CHG_I_MIN_MA;
	else
		chg_limit_current[0] = 0;
...
int smb358_gauge_get_attr_text(char *buf, int size)
{
        int len = 0;
        int soc, ibat_ma;
	struct smb358_charger *chip = the_chip;
	...
	pm8941_bms_get_batt_current(&ibat_ma);
        ...
        len += scnprintf(buf + len, size - len,
                        ...
                        ovp,
                        uvp,
                        smb358_get_prop_battery_voltage_now(the_chip),
                        ibat_ma,
                        (int)read_battery_id(the_chip),
                        smb358_get_prop_batt_temp(the_chip),
                        chip->batt_warm,
                        chip->batt_cool,
                        smb358_get_prop_batt_present(the_chip),
			pm8941_bms_get_fcc()
