...
enum {
	...
	USB_FUNCTION_PTP, /* 18 */
	USB_FUNCTION_MIDI, /* 19 */
	USB_FUNCTION_CHARGING, /* 20 */
...
int android_show_function(char *buf)
{
	...
	/* if functions are switching and show functions in simultaneously,
	 * KP will occur.
	 * Therefore, we add mutex to protect this problem.
	 */
	mutex_lock(&function_bind_sem);
...
int android_switch_function(unsigned func)
{
	...
	/* when radio flag is set to 0x20000 or debug flag is 100
	 * add mass_storage to charging and adb case for debug usage
	 */
	...
	/* when radio flag is set to 0x20000
	 * only these usb combinations can enable diag,modem,rment
	 * "mtp,adb", "mtp", "mass_storage,adb", "mass_storage"
	 */
	...
	if ((get_radio_flag() & BIT(17))) {
		if (func == ((1 << USB_FUNCTION_MTP) | (1 << USB_FUNCTION_ADB))) { /* mtp,adb */
			func |= ((1 << USB_FUNCTION_DIAG) | (1 << USB_FUNCTION_MODEM) | (1 << USB_FUNCTION_RMNET));
			pr_info("%s: add diag, modem, rment: %d\n", __func__, func);
		} else if (func == (1 << USB_FUNCTION_MTP)) { /* mtp */
			func |= ((1 << USB_FUNCTION_DIAG) | (1 << USB_FUNCTION_MODEM) | (1 << USB_FUNCTION_RMNET));
			pr_info("%s: add diag, modem, rment: %d\n", __func__, func);
		} else if (func == ((1 << USB_FUNCTION_UMS) | (1 << USB_FUNCTION_ADB))) { /* mass_storage,adb */
			func |= ((1 << USB_FUNCTION_DIAG) | (1 << USB_FUNCTION_MODEM) | (1 << USB_FUNCTION_RMNET));
			pr_info("%s: add diag, modem, rment: %d\n", __func__, func);
		} else if (func == (1 << USB_FUNCTION_UMS)) { /* mass_storage */
			func |= ((1 << USB_FUNCTION_DIAG) | (1 << USB_FUNCTION_MODEM) | (1 << USB_FUNCTION_RMNET));
			pr_info("%s: add diag, modem, rment: %d\n", __func__, func);
		} else if (func == ((1 << USB_FUNCTION_RNDIS) | (1 << USB_FUNCTION_ADB))) { /* rndis,adb*/
			func |= ((1 << USB_FUNCTION_DIAG) | (1 << USB_FUNCTION_MODEM));
			pr_info("%s: add diag, modem: %d\n", __func__, func);
		} else if (func == (1 << USB_FUNCTION_RNDIS)) { /* rndis */
			func |= ((1 << USB_FUNCTION_DIAG) | (1 << USB_FUNCTION_MODEM));
			pr_info("%s: add diag, modem: %d\n", __func__, func);
		} else
