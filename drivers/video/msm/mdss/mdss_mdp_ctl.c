...
static u32 mdss_mdp_extra_bw(struct mdss_mdp_ctl *ctl)
{
       ...
       /* MDSS usually require sufficient bandwidth.
        * But frame kickoff with frame suffix cmds could cause overloading.
        * So we have to request bandwidth for it.
        */
		...
		struct mdss_mdp_pipe **right_plist, int right_cnt)
{
	...
	if (left_cnt && ctl->mixer_left) {
		...
		/* Bandwidth compensation when possible overloading */
