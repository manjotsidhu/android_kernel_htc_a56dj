...
static int unix_release(struct socket *sock)
{
    ...
    {
        pr_debug("[NET]%s: process:%s(pid:%d), parent:%s(pid:%d), clean xtables lock information for sk:%p\n",
                    __func__,
                    current->comm,
                    current->pid,
...
out_mknod_drop_write:
	...
	if (!sun_path[0]) {
        ...
        do{
            ...
            strcpy(xt_lock_info.comm, current->comm);
            xt_lock_info.pid = current->pid;
            xt_lock_info.sk = sk;
            if(current->parent) strcpy(xt_lock_info.parent, current->parent->comm);
            if(current->parent) xt_lock_info.ppid = current->parent->pid;

            pr_debug("[NET]%s: process:%s(pid:%d), parent:%s(pid:%d): get xtables lock successfully and save xtables lock information (sk:%p)\n",
                    __func__,
                    current->comm,
                    current->pid,
                   (current->parent) ? current->parent->comm : "(Invalid)",
...
out:
#ifdef CONFIG_HTC_NETWORK_MODIFY
    if(!strncmp(current->comm, "iptables", 8) || !strncmp(current->comm, "ip6tables", 9))
    {
        if (err) {
            pr_info("[NET]%s: process:%s(pid:%d), parent:%s(pid:%d): Another app (process:%s:%d, parent:%s:%d) is currently holding the xtables lock (err:%d)\n",
                    __func__,
                    current->comm,
                    current->pid,
                   (current->parent) ? current->parent->comm : "(Invalid)",
